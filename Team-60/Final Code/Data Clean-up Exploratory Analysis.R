...
a.lm <- lm(formula = is_canceled ~ lead_time + stays_in_weekend_nights + stays_in_week_nights + required_car_parking_spaces +
...
Logmodel1 <- glm(formula = is_canceled ~ lead_time + arrival_date_year + arrival_date_month + is_Domestic + required_car_parking_spaces +
...
sum(HotelData_3$arrival_date_month == "December")
...
Logmodel2 <- glm(formula = is_canceled ~ lead_time + Season + stays_in_weekend_nights +
                   stays_in_week_nights + adults + children + is_Domestic + meal +
...
##########################################################
# Cancellations percentage rate by is_Domestic
HotelData_Final %>% group_by(is_canceled, is_Domestic) %>%
  summarise(cnt = n()) %>%
  mutate(freq = formattable::percent(cnt / sum(cnt))) %>% 
  arrange(desc(is_canceled))

City_Data %>% group_by(is_Domestic, is_canceled) %>%
  summarise(cnt = n()) %>%
  mutate(freq = formattable::percent(cnt / sum(cnt))) %>% 
  arrange(desc(is_canceled))

Resort_Data %>% group_by(is_Domestic, is_canceled) %>%
  summarise(cnt = n()) %>%
  mutate(freq = formattable::percent(cnt / sum(cnt))) %>% 
  arrange(desc(is_canceled))
...
barplot(tableSzn,beside = T, col=c("lightblue","darkgreen"),
